    ...
    def test_registered_procedure_accepts_real_image_boundary(self, tmp_path):
        ...
        script_path.write_text(
            "\n".join((
                ...
                "image_procedure = pdb.lookup_procedure('python-fu-comfyui-generate-image')",
                "assert image_procedure is not None",
                "assert image_procedure.get_menu_label() == 'Generate Image...'",
                "assert '<Image>/Tools/ComfyUI' in image_procedure.get_menu_paths()",
                "assert image_procedure.get_sensitivity_mask() == Gimp.ProcedureSensitivityMask.ALWAYS",
                ...
                "image_config = image_procedure.create_config()",
                "image_config.set_property('run-mode', Gimp.RunMode.NONINTERACTIVE)",
                "image_result = image_procedure.run(image_config)",
                "assert image_result.index(0) == Gimp.PDBStatusType.CALLING_ERROR",
