...
Migrated capabilities currently include ComfyUI health/queue/history/view/upload operations, node metadata discovery, workflow parameter preparation, LoRA chain injection, selection-mask export and injection, user workflow registration, JSON persistence, asynchronous generation coordination, cancellation, and GIMP result-layer insertion. Generated layers store JSON metadata in the versioned `comfy-data-v1` parasite; readers must tolerate its absence for older layers. The source project's richer gallery, style/history, and WebSocket-preview UI remains a follow-up migration area.